...
## 📖 项目背景
随着生成式 AI 的发展，单一的“文本到图像”模型（Text-to-Image Model）已无法满足复杂的业务需求。行业趋势正从单纯的模型训练转向 Agentic Workflow（智能体工作流）。在实际应用中，用户往往需要通过多轮对话来修正图像，或者需要生成具有连贯性（如故事绘本、分镜脚本）的内容。这要求系统具备极强的语义理解能力、任务规划能力以及工具调用能力。本项目旨在探索 LLM（大语言模型）与 Image Gen（图像生成模型）的协同机制。
